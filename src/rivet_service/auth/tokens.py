...
(docs/saas-buildout.md section 5). Hand-rolled HMAC rather than a new
dependency (itsdangerous does the same thing, but this is a small,
well-understood pattern -- see docs/prompts.md Phase 7 status).
...
def generate_token(secret_key: str, *, purpose: str, user_id: str, ttl_seconds: int) -> str:
    payload = {"purpose": purpose, "user_id": user_id, "exp": int(time.time()) + ttl_seconds}
